...
public class AlchemyScreen extends MasteryScreenBase<MasteryMenu> {

    public AlchemyScreen(MasteryMenu menu, Inventory inv, Component title) {
    ...
    protected void onClaimAll() {
        NetworkHandler.sendToServer(
                new MasteryClaimAllPacket(MasteryType.ALCHEMY)
        );
    }

    @Override
    protected Component getTitleText() {
        return Component.literal("ALCHEMY MASTERY • Level " + getPlayerLevel());
    }
    ...
    protected List<Component> getPerkTooltip(int level) {
        ...
        List<Component> rewards =
            MasteryRewardTooltip.getTooltip(MasteryType.ALCHEMY, level);
