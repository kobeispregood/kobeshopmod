...
public class FarmingScreen extends MasteryScreenBase<MasteryMenu> {

    public FarmingScreen(MasteryMenu menu, Inventory inv, Component title) {
    ...
    protected void onClaimAll() {
        NetworkHandler.sendToServer(
                new MasteryClaimAllPacket(MasteryType.FARMING)
        );
    }
    ...
    protected List<Component> getPerkTooltip(int level) {
        ...
        List<Component> rewards =
                MasteryRewardTooltip.getTooltip(MasteryType.FARMING, level);
